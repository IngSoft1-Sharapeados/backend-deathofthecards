    ...
    try:
        ...
        if partida.turno_id != id_jugador:
            raise HTTPException(
                ...
                detail=f"El jugador no esta en turno."
            )
        
        if partida.iniciada == False:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"Partida no iniciada"
            )
        ...
        if no_mas_eventos == True:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"Solo se puede jugar una carta de evento por turno."
            )
        ...
        if en_mano == False:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"La carta no se encuentra en la mano del jugador")
        ...
        if carta_evento.tipo != "Event":
            raise HTTPException(
                ...
                detail=f"La carta no es de tipo evento y no puede ser jugada como tal.")
        else:
