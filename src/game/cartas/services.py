...
class CartaService:
    ...
    def descartar_cartas(self, id_partida, id_jugador, cartas_descarte_id):
        ...
        DOC
        """
        print("ACA POR AGARRAR LA PARTIDA")
        partida = PartidaService.obtener_por_id(id_partida)
        print("YA AGARRE LA PARTIDA")
        print("ACA POR AGARRAR EL JUGADOR")
        jugador = JugadorService.obtener_jugador(id_jugador)
        print("YA AGARRE EL JUGADOR")

        if (partida.turno_id != jugador.id):
            raise Exception("No es el turno del jugador")
        
        else:
            # ahora quiero verificar que la lista de cartas esten en la mano del jugador
